...
contract Anchor {
    ...
    struct Application {
        address user;
        string[] docCids;     // IPFS CIDs of uploaded documents
        uint256 timestamp;
        bool processed;
    }
    ...
    event ApplicationSubmitted(address indexed user, string[] docCids, uint256 indexed applicationId, uint256 timestamp);
    ...
    // --- User KYC Submission ---
    function applyForPassport(string[] calldata docCids) external {
        require(docCids.length > 0, "No documents provided");
        ...
        emit ApplicationSubmitted(msg.sender, docCids, appId, block.timestamp);
    }
    ...
    function getApplication(uint256 appId) external view returns (
        ...
        return (app.user, app.docCids, app.timestamp, app.processed);
    }
